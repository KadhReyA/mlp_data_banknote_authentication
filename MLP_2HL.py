# Menggunakan basis code milik : Muhammad Raihan Wahyu Yuwono
# Link Github : https://github.com/raihanyuwono
# Dilakukan pengubahan nilai hidden layer, nilai learning rate, dan nama variabel beberapa fungsi
...
fig = plt.figure(0)
fig.canvas.set_window_title('Cost Function Graph')
...
class Classifier:
    ...
    def __backpropagation(self, data, fact):
        shift = [[] for _ in range(self.__layers)]
        for layer in range(self.__layers - 1, -1, -1):
            for neuron in range(self.__neurons[layer]):
                prediction = self.__predictions[layer][neuron]
                # Output to Hidden Layer
                if layer == self.__layers - 1:
                    shift[layer].append(self.__shift_function(prediction, fact=fact))
                # Hidden Layer to Hidden/Input Layer
                else:
                    total = 0.0
                    for next_neuron in range(self.__neurons[layer + 1]):
                        total += shift[layer + 1][next_neuron] * self.__weights[layer + 1][next_neuron][neuron]
                    shift[layer].append(self.__shift_function(prediction, total=total))
                data_used = data if layer == 0 else self.__predictions[layer - 1]
                # Update Weights
                for i, data_prev in enumerate(data_used):
                    self.__weights[layer][neuron][i] -= self.__learning_rate * self.__delta_function(shift[layer][neuron], data_prev)
                # Update Bias
                    self.__biases[layer][neuron] -= self.__learning_rate * self.__delta_function(shift[layer][neuron], 1)

    def __delta_function(self, shift, data):
        return shift * data    

    def __shift_function(self, prediction, fact=None, total=None):
        if total == None:
...
def main():
    ...
    # Classifier
    clf = Classifier()
    clf.train(train.data, train.fact, epoch=50, learning_rate=0.001, hidden_layer=[2, 2])
